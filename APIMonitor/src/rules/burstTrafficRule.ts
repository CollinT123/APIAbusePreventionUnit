...
export function burstTrafficRule(input: {
  ...
  return {
    matched: true, ruleName: "burstTraffic", severity: config.burstTraffic.severity,
    ...
    evidence: { fingerprint: current.fingerprint, sourceKey: currentSourceKey, occurrencesIncludingCurrent, windowMs: config.burstTraffic.windowMs }
  };
}
