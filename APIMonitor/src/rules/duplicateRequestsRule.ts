...
export function duplicateRequestsRule(input: {
  ...
  return {
    ...
    reason: `Duplicate API requests from the same source detected (${occurrencesIncludingCurrent} in ${Math.max(1, lastMs - firstMs)}ms).`,
    recommendedAction: config.recommendedActions.duplicateRequests,
    evidence: { fingerprint: current.fingerprint, sourceKey: currentSourceKey, occurrencesIncludingCurrent, windowMs: config.windowsMs.duplicates }
  };
}
