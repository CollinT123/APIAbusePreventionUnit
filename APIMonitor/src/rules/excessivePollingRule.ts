...
export function excessivePollingRule(input: {
  ...
  const allEvents = [...relevant, current].sort((a, b) => parseIsoTimestampToMs(a.timestamp) - parseIsoTimestampToMs(b.timestamp));
  ...
  if (occurrencesIncludingCurrent >= config.excessivePolling.minOccurrencesForOrange && minIntervalMs <= config.excessivePolling.minIntervalMsForOrange) {
    return { matched: true, ruleName: "excessivePolling", severity: "ORANGE",
      ...
      evidence: { fingerprint: current.fingerprint, sourceKey: currentSourceKey, occurrencesIncludingCurrent, minIntervalMs, windowMs: config.windowsMs.polling } };
  }
  if (occurrencesIncludingCurrent >= config.excessivePolling.minOccurrencesForYellow && minIntervalMs <= config.excessivePolling.minIntervalMsForYellow) {
    return { matched: true, ruleName: "excessivePolling", severity: "YELLOW",
